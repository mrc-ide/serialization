...
import org.mrc.ide.serialization.FlexibleDataTable
import org.mrc.ide.serialization.models.FlexibleProperty

class FlexibleDataTableTests {
    ...
    fun `headers are written in order of constructor`() {
        val table = FlexibleDataTable.new<ABC>(emptySequence(), listOf(), DefaultSerializer.instance)
        Assertions.assertThat(serialize(table)).isEqualTo("""a,b""")
    }

    @Test
    fun `throws error if no property marked as flexible`() {
        Assertions.assertThatThrownBy { FlexibleDataTable.new<XYZ>(emptySequence(), listOf(), DefaultSerializer.instance) }
    ...
    fun `throws error if flexible property is not a map`() {
        Assertions.assertThatThrownBy { FlexibleDataTable.new<DEF>(emptySequence(), listOf(), DefaultSerializer.instance) }
    ...
    fun `extra headers are written at the end`() {
        val table = FlexibleDataTable.new<ABC>(emptySequence(), listOf("extra1", "extra2"), DefaultSerializer.instance)
    ...
    fun `data is written out line by line`() {
        ...
        val table = FlexibleDataTable.new<ABC>(data, listOf("extra1", "extra2"), DefaultSerializer.instance)
...
x,y,<NA>,w""")
    ...
    fun `flexible values do not get serialised if header not explicitly provided`() {
        ...
        val table = FlexibleDataTable.new<ABC>(data, listOf("extra1"), DefaultSerializer.instance)
